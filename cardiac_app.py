...
st.caption("Patient-profiled vitals • Auto/manual input • Trend visualization")
...
weight = st.number_input("Weight (kg)", 2.0, 250.0, 70.0)
...
if mode == "🔁 Auto Mode":
    if refresh_count > 0:
        ...
        save_reading(ts, pid, fname, lname, dob.isoformat(), age, consultant, location,
                     weight, height, bmi, spo2, hr, ecg, score, level)
...
# ========== Patient History ==========
st.markdown("---")
st.markdown(f"### 📋 Readings for `{pid}`")
df = load_history(pid)
...
if df.empty:
    ...
    st.download_button("📤 Export CSV", data=csv, file_name=f"{pid}_readings.csv", mime="text/csv")

    st.markdown("### 📈 Risk Trend")
    df["timestamp"] = pd.to_datetime(df["timestamp"])
    df = df.sort_values("timestamp")
    df["color"] = df["total_risk"].apply(lambda s: "#21BF73" if s == 0 else
                                                   "#FFC300" if s <= 2 else
                                                   "#FF6F00" if s <= 5 else
                                                   "#C70039")
